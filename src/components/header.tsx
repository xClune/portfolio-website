export default function Header() {
  return (
    <div className="p-6 md:ml-5 md:mr-10">
      <div className="flex flex-col font-mono text-white gap-2 justify-center text-center md:text-right md:items-end md:justify-end md:mt-5">
        <h1 className="text-2xl font-bold">Adam Clune</h1>
        <h2 className="text-lg">CIVIL | MECHANICAL | SOFTWARE</h2>
        <h2 className="text-lg">Engineering & Project Management</h2>
      </div>
      <div className="border-b border-white mt-4"></div>
    </div>
  );
}
